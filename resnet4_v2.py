...
plot_save_dir = r'checkpoints\resnet4_v2'
summary_dir = 'summaries/' + "model4_v2"
...
config = {
    ...
    "workers": 8 if device.type == "cuda" else 0,  # Number of data loading workers
    ...
    "save_dir": "checkpoints/resnet4_v2",  # Directory to save trained models
    "save_every": 1,  # Save checkpoints every specified number of epochs
}
...
CIFAR10_MEAN = (0.4914, 0.4822, 0.4465)  # CIFAR-10 specific
...
def getDataLoaders(train_data_dir, test_pkl_dir, batch_size_in, train_tfms, valid_tfms):
    ...
    # Create DataLoaders
    train_loader = DataLoader(full_train_dataset, batch_size=batch_size_in, shuffle=True, pin_memory=pin_Mem, num_workers=config['workers'], persistent_workers=True)
    ...
    valid_loader = DataLoader(valid_dataset, batch_size=int(batch_size_in / 4), shuffle=False, pin_memory=pin_Mem, num_workers=0)
    ...
    test_loader = DataLoader(test_dataset, batch_size=batch_size_in, shuffle=False, pin_memory=pin_Mem, num_workers=0)
...
def resnetCustom():
    return ResNet(BasicBlock, [50, 50, 50])
...
def main():
    ...
    for i, ax in enumerate(axes.flat):
        ...
        img = img * torch.tensor(CIFAR10_MEAN).to(img.dtype) + torch.tensor(CIFAR10_STD).to(img.dtype)
    ...
    model = ResNet(BasicBlock, [50, 50, 50])
    total_params = 0
    ...
    for epoch in range(0, config['epochs']):
        ...
        if epoch > 0 and epoch % config['save_every'] == 0:
            ...
            save_checkpoint({
                'state_dict': model.state_dict(),
                'best_prec1': best_prec1,
            }, is_best, filename=os.path.join(config['save_dir'], 'resnet4_v2.th'))
    ...
    # Save to CSV file
    submission_df.to_csv('submissionTest6_resnet110.csv', index=False)

    print("Submission file saved as 'submissionTest6_resnet110.csv'")
    ...
    checkpoint_path = os.path.join(config['save_dir'], 'resnet4_v2.th')
    ...
    for i, ax in enumerate(axes.flat):
        ...
        img = img * torch.tensor(CIFAR10_MEAN).to(img.dtype) + torch.tensor(CIFAR10_STD).to(img.dtype)  # Denormalization
