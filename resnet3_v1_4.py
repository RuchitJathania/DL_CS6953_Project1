...
writer = SummaryWriter('summaries/' + "resnet3_v1_4")
...
config = {
    ...
    'grad_clip': 0.1,
    'lookahead': 1,
    'save_dir': os.path.join("checkpoints", "resnet3_v1_4")
}
...
transform_train = tt.Compose([
    ...
    tt.RandomAffine(0, shear=4, scale=(0.9, 1.1)),
    tt.RandomAffine(0, translate=(0.1, 0.1)),
    tt.ColorJitter(brightness=0.17, contrast=0.17, saturation=0.17),  # had hue=0.5 before
...
def train(epoch, config, train_loader, model, optimizer, criterion):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        unique_labels = torch.unique(targets)  # Get unique class labels in batch
        num_classes_in_batch = unique_labels.numel()  # Count number of unique labels
        cutmix = tt.CutMix(num_classes=num_classes_in_batch)
        mixup = tt.MixUp(num_classes=num_classes_in_batch)
        cutmix_or_mixup = tt.RandomChoice([cutmix, mixup])
        ...
        # Convert one-hot or soft labels to class indices
        if targets.ndim == 2:  # If targets is (batch_size, num_classes), convert it
...
def test(epoch, config, savename, valid_loader, model, criterion):
    ...
    if acc > best_acc:
        ...
        torch.save(state, os.path.join('./checkpoints/', savename, 'resnet3_v1_4.th'))
        best_acc = acc


def save_checkpoint(epoch, model, config, acc, save_path="checkpoint.th"):
    checkpoint = {
        'epoch': epoch,
        'state_dict': model.state_dict(),
        'acc': acc,
...
def load_checkpoint(model, optimizer, checkpoint_path="checkpoint.th"):
...
def main():
    ...
    if config['resume_ckpt']:
        start_epoch, _ = load_checkpoint(model, optimizer, checkpoint_path="checkpoints/resnet3_v1_4/resnet3_v1_4.th")
    else:
    ...
    for epoch in range(start_epoch, config["max_epochs"]):
        ...
        test(epoch, config, savename="resnet3_v1_4", valid_loader=valid_loader, model=model, criterion=criterion)
        save_checkpoint(epoch, model, config, best_acc, save_path="checkpoints/resnet3_v1_4/resnet3_v1_4.th")
        scheduler.step()
    ...
    submission_df.to_csv('submissionTest11.csv', index=False)

    print("Submission file saved as 'submissionTest11.csv'")
    ...
    checkpoint_path = os.path.join(config['save_dir'], 'resnet3_v1_4.th')
    checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False)
