...
writer = SummaryWriter('summaries/' + "model3_v2")
...
# Albumentations train transformation
train_transform = A.Compose([
    A.PadIfNeeded(min_height=36, min_width=36, p=1.0),  # Equivalent to padding
    A.RandomCrop(width=32, height=32, p=1.0),
    A.HorizontalFlip(p=0.5),  # 50% chance to flip
    A.ShiftScaleRotate(shift_limit=0.1, scale_limit=0.1, rotate_limit=10, p=0.5),
    A.RandomBrightnessContrast(brightness_limit=0.2, contrast_limit=0.2, p=0.5),
    A.GaussianBlur(blur_limit=(3, 5), p=0.3),
    A.GaussNoise(std_range=(.01, .05), mean_range=(0, 0), p=0.3),
    A.CoarseDropout(num_holes_range=(1, 1), hole_height_range=(1, 8), hole_width_range=(1, 8), p=0.5),
    A.Normalize(mean=CIFAR10_MEAN, std=CIFAR10_STD),  # Normalization at the end
...
# train_trans = [tt.ToTensor()]
# test_trans = [tt.ToTensor()]
# if config["data_augmentation"]:
#     train_trans.append(tt.RandomCrop(32, padding=4))
#     train_trans.append(tt.RandomHorizontalFlip())
# if config["data_normalize"]:
#     train_trans.append(tt.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)))
#     test_trans.append(tt.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)))
# transform_train = tt.Compose(train_trans)
# transform_test = tt.Compose(test_trans)
...
if __name__ == "__main__":
    train_loader, valid_loader, test_loader = getDataLoaders(cifar10_dir, test_data_dir, config['batch_size'],
                                                             train_tfms=train_transform, valid_tfms=test_transform)
    ...
    for epoch in range(start_epoch, config["max_epochs"]):
        ...
        test(epoch, config, savename="model3_v2")
        scheduler.step()
    ...
    # Save to CSV file
    submission_df.to_csv('submissionTest3.csv', index=False)

    print("Submission file saved as 'submissionTes3.csv'")
